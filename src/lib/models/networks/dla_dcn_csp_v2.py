...
#@zenghui change BasicBlock to yolo5 C3
...
class C3(nn.Module):
    ...
    def __init__(self, c1, c2, stride=1,dilation=1, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        super(C3, self).__init__()
        c_ = int(c2 * e)  # hidden channels
        # self.cv1 = Conv(c1, c_, 1, 1)
        # self.cv2 = Conv(c1, c_, 1, 1)
        self.cv1 = Conv(c1, c_, 1, stride)
...
#             y=self.m(self.cv1(x))
#             print(x.shape, self.cv2(x).shape, residual.shape)
...
class C3_Root(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, residual,stride=1, dilation=1, shortcut=True, g=1, e=0.5):
        ...
        # self.cv1 = Conv(c1, c_, 1, 1)
        # self.cv2 = Conv(c1, c_, 1, 1)
        self.cv1 = Conv(in_channels, c_, 1, stride)
        self.cv2 = Conv(in_channels, c_, 1, stride)
        n=stride
        self.m = nn.Sequential(*[C3Bottleneck(c_, c_, shortcut, g, e=1.0) for _ in range(n)])
        # self.m = nn.Sequential(*[CrossConv(c_, c_, 3, 1, g, 1.0, shortcut) for _ in range(n)])
        self.cv3 = Conv(2 * c_, out_channels, 1)  # act=FReLU(c2)
        self.act = nn.SiLU()
...
def get_dla_csp_v2(num_layers, heads, head_conv=256, down_ratio=4):
